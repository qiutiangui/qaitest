...
@router.post("/index/feishu")
async def index_feishu_document(
    project_id: int = Form(..., description="项目ID"),
    version_id: Optional[int] = Form(None, description="版本ID"),
    doc_url: str = Form(..., description="飞书文档URL"),
    requirement_name: Optional[str] = Form(None, description="需求名称（用于关联需求）"),
    chunk_size: int = Form(500, description="分块大小"),
    overlap: int = Form(100, description="块重叠大小"),
):
    """
    从飞书云文档索引内容
    ...
    try:
        ...
        if not app_id or not app_secret:
            raise HTTPException(
                status_code=500,
                detail="飞书应用未配置，请设置 FEISHU_APP_ID 和 FEISHU_APP_SECRET"
            )
        ...
        # 加载文档
        docs = await reader.load_data_from_url(doc_url)

        if not docs:
            raise HTTPException(status_code=404, detail="无法获取飞书文档内容")

        document = docs[0]
        filename = document.metadata.get("title", "feishu_document")
        ...
        if result.get("success"):
            return {
                "success": True,
                "message": f"成功从飞书文档索引 {result.get('indexed', 0)} 个文本块",
                "source": "feishu",
                "doc_title": filename,
                "data": result
            }
...
async def get_feishu_config_status():
    ...
    return {
        ...
        "app_id": app_id[:8] + "***" if app_id else None,
        "message": "已配置" if app_id else "未配置，请在 .env 中设置 FEISHU_APP_ID 和 FEISHU_APP_SECRET"
    }
