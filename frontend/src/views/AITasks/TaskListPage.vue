...
const stats = ref<Stats>({ total_running: 0, total_completed: 0, total_failed: 0 })
...
const filteredTasks = computed(() => {
  ...
  if (searchKeyword.value) {
    ...
    result = result.filter(t => 
      ...
      t.requirement_name?.toLowerCase().includes(keyword) ||
      t.task_id.toLowerCase().includes(keyword) ||
      t.current_phase?.toLowerCase().includes(keyword)
    )
  }
...
const viewDetail = (task: TaskRecord) => {
  router.push(`/tasks/${task.task_type}/${task.task_id}`)
}
...
const handleDelete = async (task: TaskRecord, e: Event) => {
  ...
  try {
    ...
    await api.delete(`/tasks/${task.task_type}/${task.task_id}?force=${isForce}`)
...
const handleCancel = async (task: TaskRecord, e: Event) => {
  ...
  try {
    ...
    await api.post(`/tasks/${task.task_type}/${task.task_id}/cancel`)
  ...
  <div class="task-list-page p-6 max-w-7xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
      <div class="flex items-center gap-3">
        ...
        <div>
          <h1 class="text-2xl font-bold text-gray-900">AI任务</h1>
          <p class="text-sm text-gray-500 mt-0.5">查看和管理所有AI任务执行情况</p>
        </div>
      </div>
    ...
    <div class="grid grid-cols-3 gap-4 mb-6">
      <div class="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-gray-900">{{ stats.total_running }}</p>
        </div>
      </div>
      ...
      <div class="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
        ...
        <div>
          ...
          <p class="text-2xl font-bold text-gray-900">{{ stats.total_failed }}</p>
        </div>
      </div>
    </div>
        ...
        <div class="flex items-start gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-2">
              ...
              <span :class="[
                ...
                task.task_type === 'requirement' 
                  ? 'bg-purple-100 text-purple-700' 
                  : 'bg-teal-100 text-teal-700'
              ]">
                {{ task.task_type === 'requirement' ? '需求分析' : '用例生成' }}
              </span>
              
              <!-- 项目 -->
              <span class="font-semibold text-gray-900 truncate">{{ task.project_name }}</span>
              
              <!-- 需求名称 -->
              <span v-if="task.requirement_name" class="text-gray-500 truncate">
                / {{ task.requirement_name }}
              </span>
            </div>
            
            <!-- 阶段进度 -->
            <div v-if="task.phases_summary?.length" class="flex items-center gap-2 mb-2">
              <div 
                v-for="(phase, idx) in task.phases_summary" 
                :key="phase.code"
                class="flex items-center gap-1.5"
              >
                <component 
                  :is="getPhaseIcon(phase)" 
                  :class="['w-3.5 h-3.5', getPhaseStatusColor(phase)]"
                />
                <span class="text-xs text-gray-500">{{ phase.name }}</span>
                <span v-if="idx < task.phases_summary.length - 1" class="text-gray-300">›</span>
              </div>
            </div>
            
            <!-- 当前阶段 -->
            <div v-if="task.current_phase && task.status === 'running'" class="flex items-center gap-2 mb-2">
              <span class="animate-pulse text-blue-500">●</span>
              <span class="text-sm text-blue-600">{{ task.current_phase }}</span>
            </div>
          ...
          >
            <button
              ...
              class="p-2 text-orange-500 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors"
              title="取消任务"
            >
          ...
          <div 
            ...
            :class="(task.status === 'running' || task.status === 'pending') ? 'group-hover:gap-2 opacity-0 group-hover:opacity-100' : ''"
          >
            <button
              @click="viewDetail(task)"
